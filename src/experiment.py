...
@ex.main
def run(batch_size, max_batch_steps, z_dim, dropout_rate, rnn_dim, rnn_layers, num_iafs, iaf_dim, transition_dim,
        channel, init_lr, epochs, annealing_epochs, temp, min_af, _run: "Run"):
    ...
    # dataset
    wildfire_data = WildFireDataset(train=True, config_file="config.ini")
    data_loader = DataLoader(wildfire_data, batch_size=batch_size, shuffle=True, num_workers=4)
    expected_batch_size = np.ceil(len(wildfire_data) / batch_size)
    expected_batch_size = max_batch_steps if max_batch_steps > 0 else expected_batch_size
    vae_config = VAEConfig(z_dim=z_dim, image_dim=(30, 30), dropout_rate=dropout_rate, rnn_dim=rnn_dim,
                           rnn_layers=rnn_layers, num_iafs=num_iafs, iaf_dim=iaf_dim, transition_dim=transition_dim,
                           channel=channel, init_lr=init_lr)
    ...
    for step in trange(epochs, desc="Epoch: ", ascii=False, dynamic_ncols=True,
        ...
        for batch_steps_i, d in tqdm(enumerate(data_loader), desc="Batch: ", ascii=False, dynamic_ncols=True,
                                     ...
                                     leave=False):
            epoch_elbo += svi.step(_ct(d.diurnality), _ct(d.viirs), annealing_factor)
            epoch_time_slices += d.viirs.shape[0] * d.viirs.shape[0]
            if 0 < max_batch_steps == batch_steps_i:
                break
        _run.log_scalar("elbo", epoch_elbo / epoch_time_slices, step=step)
